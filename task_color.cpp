...
QString Task_Color::taskName = QSL("颜色追踪");

Task_Color::Task_Color(int period, QObject *parent) :
	Task(period, parent),
	GoCtrl(1.05, 0, 0.8),
	XAdjustCtrl(0.8, 10000, 0.3)
{
	connect(this, &Task_Color::videoImgIn, this, &Task_Color::camIdentifyTarget);
...
void Task_Color::setTaskPara()
{

}

void Task_Color::taskSetUp()
{
	//检查一下相机
	if(Vision::fCamHdl)
	{
	...
	//第一个目标函数
	pCtrlFunc = &Task_Color::ctrlSetSearchDepth;
	...
	connect( timer, &QTimer::timeout, this, &Task_Color::ctrlTimerUpdate, Qt::QueuedConnection);
...
void Task_Color::ctrlTimerUpdate()
{
	if(pCtrlFunc)
...
void *Task_Color::ctrlSetSearchDepth(void *)
{
	...
	pCtrlFunc = &Task_Color::ctrlWaitForDepthOK;
...
void *Task_Color::ctrlWaitForDepthOK(void *)
{
	taskStatus = QSL("%1").arg((QString)__FUNCTION__);
	...
	if(isLightInSight)//等待定深的过程中就找到了
	{
		...
		pCtrlFunc = &Task_Color::ctrlChaseTarget;
...
void *Task_Color::ctrlChaseTarget(void *)
{
...
void Task_Color::camIdentifyTarget(Mat src)
{
	Mat canImage, img_YH, img_YS, img_Y_detected;
	bool flag = false;
	...
	//进行形态学开运算操作
	morphologyEx(img_Y_detected, img_Y_detected, MORPH_OPEN, element);
	Canny(img_Y_detected, canImage, thresh, thresh * 3, 3);

	int P_x_b = 0, P_x_e = 0, P_y_b = 0, P_y_e = 0;
	float maxarea = 0;
	int Foundtime = 0;
	Vision::rect_boundary_light(canImage, src, flag, maxarea, Foundtime, P_x_b, P_x_e, P_y_b, P_y_e);
	emit videoImg(src);
	...
	double dest_x = A.x - src.cols / 2;
	double dest_y = (A.y - src.rows / 2);
